...
@export var damage_interval := 1  # Intervalo entre cada daño al jugador
...
func _physics_process(delta):
	if is_alive:
		...
		if is_attacking and is_within_attack_range():
			damage_timer -= delta
			if damage_timer <= 0:
...
func start_attack():
	if is_attacking:
		return  # Previene ataques repetidos innecesarios.
	is_attacking = true
