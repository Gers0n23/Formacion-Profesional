...
def main(page: ft.Page):
    ...
    def button_1():
        return ft.Container(
                            ft.ElevatedButton(text="Generar Venta",on_click=generar_venta,icon=ft.icons.TRANSIT_ENTEREXIT,style=ft.ButtonStyle(
                                                         shape=ft.RoundedRectangleBorder(radius=8))),
    ...
    def agregar_venta_actual(e):
        ...
        nombre, precio_un = busqueda(codigo)
        ...
        tabla = current_sale()
        # Aquí necesitas reemplazar la tabla antigua con la nueva tabla
        total_valor_total = sum(item['valor_total'] for item in venta_actual)
        subtotal1= int(total_valor_total) 
        iva = int(subtotal1 * 0.19)
        total = int(subtotal1 + iva)
        print(subtotal1)
        total_current_sale()
        page.update()
        return subtotal1
    ...
    def generar_venta(e):
        ...
        for producto in venta_actual:
            ...
            # Agregar la nueva venta a rows aquí
            cells = [
                ft.DataCell(ft.Text(codigo)),
                ft.DataCell(ft.Text(nombre)),
                ft.DataCell(ft.Text(cantidad)),
            ...
            rows.append(ft.DataRow(cells=cells))
        venta_actual=[]
        current_sale()  # Llama a current_sale() después de que todas las ventas se hayan agregado a rows
        page.update()
        print(rows)
        return historico_ventas    
    
    def total_current_sale():
        global subtotal1
        global subtotal_final
        global iva
        global total

        subtotal_final=subtotal1
        print(subtotal_final)
        iva = int(subtotal_final * 0.19)
        total = int(subtotal_final + iva)
        page.update()
        return ft.Row([ ft.Text(f"Subtotal: $ {int(subtotal_final)}", width=180, text_align=ft.TextAlign.CENTER),
    ...
    def full_sales_panel():
        ...
        return ft.Column(
                    [   
                        current_sale(),
                        total_current_sale(),  # Llama a total_current_sale() aquí para actualizar los valores
                        button_1(),
                    ]
                    )
    ...
    page.add(
        ...
        ft.Row(
            [
                ...
                full_sales_panel(), #Panel de ventas          
            ],
            spacing=10,
